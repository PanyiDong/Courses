...
for i in range(sample) :
    time[i] = int(float(dataset[i][1]))
    left_steps[i] = int(dataset[i][2])
    right_steps[i] = int(dataset[i][3])
    left_time[i] = int(dataset[i][4])
    right_time[i] = int(dataset[i][5])
    escape_direction[i] = int(dataset[i][6])
